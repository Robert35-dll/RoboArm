...
float UpperAngles[AxisAmount];  // The angles of upper arm
float LowerAngles[AxisAmount];  // The angles of underarm
...
void GetAngles(int, float[3], bool);
void CorrectAngles(int, float[3]);
void GetRawAccelerations(int);
void GetRawRotations(int);
void PrintData();
...
void GetAngles(int mpuNum, float (outArr)[3], bool corrReq) {
    ...
    if (corrReq) {
        ...
        outArr[0] = AccAngs[mpuNum][0];
        outArr[1] = AccAngs[mpuNum][1];
        outArr[2] = AccAngs[mpuNum][2];
    }
}
void CorrectAngles(int mpuNum, float outArr[3]) {
    ...
    outArr[0] = Alphas[mpuNum] * (outArr[0] + (Rots[mpuNum][0] - GyroBiases[mpuNum][0]) * TIMEOUT / 1000)
                 + (1 - Alphas[mpuNum]) * (AccAngs[mpuNum][0] + GyroCorrs[mpuNum][0]);
    outArr[1] = Alphas[mpuNum] * (outArr[1] + (Rots[mpuNum][1] - GyroBiases[mpuNum][1]) * TIMEOUT / 1000)
                 + (1 - Alphas[mpuNum]) * (AccAngs[mpuNum][1] + GyroCorrs[mpuNum][1]);
    outArr[2] = Alphas[mpuNum] * (outArr[2] + (Rots[mpuNum][2] - GyroBiases[mpuNum][2]) * TIMEOUT / 1000)
                 + (1 - Alphas[mpuNum]) * (AccAngs[mpuNum][2] + GyroCorrs[mpuNum][2]);
}

void GetRawAccelerations(int mpuNum) {
...
void GetRawRotations(int mpuNum) {
    Wire.beginTransmission(Addresses[mpuNum]);
    Wire.write(0x43);
    Wire.endTransmission(true);
    Wire.requestFrom(Addresses[mpuNum], 6, true);
